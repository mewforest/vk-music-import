...
| `ADD_TO_GROUP_ID`       | Добавлять треки в сообщество с указанным ID. Если пусто, то опция выключена.                               | ``                    |
...
### Добавление в сообщество (группу)

Чтобы добавить треки в сообщество (группу), вы должны быть её администратором. В `config.env` добавьте строчку: 
`ADD_TO_GROUP_ID="123456789"`. Вместо`123456789` укажите ID сообщества ([как узнать id группы в VK](https://vk.com/faq18062)), 
